...
const archiveProcess = async () => {
    ...
    (await config()).serviceDefiPricesArchive && await archivePrices(prices);
};
...
const initSubmit = async (acc, pubKey, pvtKey, options = {}) => {
    ...
    if ((await config()).serviceDefiPricesSubmit) {
...
const initArchive = async (prsCallback, options = {}) => {
    ...
    silent = !!options.silent;
    if ((await config()).serviceDefiPricesWatch) {
        return await (options && options.event || event).loop(
...
const { utilitas, event, math, fetch } = require('utilitas');
