...
const [defaultSlippage, swapFeeRate, precision] = [5, 0.003, 8];
...
const [cSwap, cOrc, defaultTransMemo, addLiquidMemo, rmLiquidMemo, plErr] = [
    'prs.swap', 'swap.oracle', 'Swap tokens via PRESS.one',
    'Add swap liquid to PRESS.one', 'Remove swap liquid from PRESS.one',
    'No pool is available to swap'
];
...
const assertPool = (p) => { utilitas.assert(p && p.length, `${plErr}.`, 500); };
const fT = async (c, t, f) => { return (await table.getAll(c, t)).map(f); };
const getAllPools = async () => { return await fT(cSwap, 'pool', packPool); };
const getAllTrxs = async () => { return await fT(cSwap, 'trxs', packTrx); };
const getAllTrans = async () => { return await fT(cOrc, 'trans', packTran); };
...
const assertMatchedPool = (pool, cA, cB) => {
    utilitas.assert(pool, `${plErr} between ${cA} and ${cB}.`, 400);
};
...
const packTrx = (p) => {
    ...
    p.req_timestamp = utilitas.getDateByUnixTimestamp(p.req_timestamp);
    return p;
};

const packTran = (p, options) => {
    options = options || {};
    p.mixin_trace_id = [];
    for (let i = 0; i < 4; i++) {
        p.mixin_trace_id.push(
            mixin.getTraceByTransactionId(p.trx_id_received, i)
        );
    }
    p.type = getTranType(p.type);
    p.status = getTranStatus(p.status);
    p.timestamp_received = utilitas.getDateByUnixTimestamp(
    ...
    for (let i = 0; i < 2; i++) {
        ...
        if (!plan[2]) { continue; }
        const pI = i + (p.type === 'SETTLE' ? 2 : 0);
        p.payment_request[p.mixin_trace_id[pI]] = {
            currency: plan[1],
            amount: plan[0],
            payment_url: p.type === 'COLLECT'
                ? mixin.createPaymentUrlToOfficialAccount(
                    plan[0], p.mixin_trace_id[pI], p.memo || defaultTransMemo,
                    { currency: plan[1], account: options.account || null }
                ) : null,
        };
    }
    return p;
...
const getPoolByCurrency = async (currencyA, currencyB) => {
    currencyA = mixin.assetAssetSymbol(currencyA);
    currencyB = mixin.assetAssetSymbol(currencyB);
    let [pair, pol, ctch] = [[currencyA, currencyB], await getAllPools(), null];
    assertPool(pol);
    for (let x of pol) {
        if (utilitas.arrayEqual(pair, x.tokens.map(y => { return y.symbol }))) {
            ctch = x; break;
        }
    };
    assertMatchedPool(ctch, currencyA, currencyB);
    return ctch;
};
