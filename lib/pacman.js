...
const saveTransactions = async (block) => {
    ...
    for (let i of block.transactions
        ...
        // search index patch {
        enriched.transactions_trx_transaction_actions_data_data_allow = p?.allow;
        enriched.transactions_trx_transaction_actions_data_data_deny = p?.deny;
        enriched.transactions_trx_transaction_actions_data_data_file_hash = p?.file_hash;
        // }
...
const analytics = async () => {
    let [fields, groups, transactions, blocks] = [[
        'transactions_trx_transaction_actions_account',
        'transactions_trx_transaction_actions_name',
        'transactions_trx_transaction_actions_data_type',
    ].join(', '), [], 0];
    (await database.query(
        ...
        if (x.transactions_trx_id === '*') {
            blocks = Number(x.block_num);
        } else {
            transactions += (x.count = Number(x.count));
            delete x.transactions_trx_id;
            delete x.block_num;
            groups.push(x);
        }
    });
    return { groups, transactions, blocks };
};
