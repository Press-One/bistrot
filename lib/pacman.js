...
const [_declaration, aFLT, aSTG, aLNG, aDTE, aUTK, aSTR, aTXT] = [
    makeAttrs({ version, encoding }), makeAttrType('float'), makeAttrText(),
    makeAttrType('long'), makeAttrType('date'), makeAttrText({ index: [DOCS] }),
    makeAttrText({ index: [DOCS], token: true }), makeAttrText({
...
const packAction = (action, transaction, block, actIdx) => {
    ...
    if (xmlJson) {
        act = buildXmlJson(act, {
            ...
            URI: `${uriRoot}transactions/${act[key('TRANSACTION_ID')]}/${actIdx}`,
        });
