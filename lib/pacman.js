...
const uriRoot = 'https://prs-bp1.press.one/api/chain/';
...
const rawVerifyDelay = 10 + 1; // 10 seconds
...
const getVerifyDelay = () => { return (current / inteval) * rawVerifyDelay; };
...
const makeAttrText = (def) => {
    const atts = {
        type: `${idxSys}.text`, token: false, sort: false, trimhtml: false,
        tv: false, index: ['docs'].join(','), store: true, ...def || {},
    };
    for (let i in atts) { if (!atts[i]) { delete atts[i]; } }
    return makeAttrs(atts);
};

const [_declaration, aUTK, aLNG, aFLT, aDTE, aSTR, aTXT] = [
    makeAttrs({ version, encoding }), makeAttrText(), makeAttrType('long'),
    makeAttrType('float'), makeAttrType('date'), makeAttrText({ token: true }),
    makeAttrText({
        token: true, trimhtml: true,
        index: ['docs', 'offset', 'freq', 'pos'].join(','),
    })
];
...
const valuePack = (k, v) => {
    v = v ?? '';
    [ORI, JSN].includes(rule(k)) || (v = utilitas.ensureArray(v).join(' '));
    switch (rule(k)) {
        case BLN: v = { [_text]: String(!!v).toUpperCase(), ...aUTK }; break;
        case ENM: v = { [_text]: v.toUpperCase(), ...aUTK }; break;
        case JSN: v = JSON.stringify(v); case CDT: v = { [_cdata]: v }; break;
        case LNG: v = { [_text]: utilitas.ensureInt(v), ...aLNG }; break;
        case ORI: break;
        case STR: case STA: v = { [_text]: v, ...aSTR }; break;
...
const packIndex = (action, transaction, block) => {
    const k = action?.data;
    ...
    if (xmlJson) {
        act = buildXmlJson(act, {
            ...
            TITLE: `Transaction ${act[key('TRANSACTION_ID')]}`,
            URI: `${uriRoot}transactions/${act[key('TRANSACTION_ID')]}`,
        });
        block = buildXmlJson(block, {
            CLASS: classBlack, CONTENT: null,
            TITLE: `Block ${block[key('BLOCK_ID')]} (${block[key('BLOCK_NUM')]})`,
            URI: `${uriRoot}blocks/${block[key('BLOCK_ID')]}`,
...
const analytics = async () => {
    ...
    log(arrLog.join(', ') + '.');
    // verify
    const [missing, lastVerifiedId, verifiedPercentage, verifiedRange] = [[]];
    if (utilitas.checkInterval(rawVerifyDelay)
        && callbacks.verifiedIdGet
        && callbacks.verifiedIdSet) {
    ...
    return {
        ...
        verified: v, lastVerifiedId, verifiedPercentage, verifiedRange, missing,
    };
};
