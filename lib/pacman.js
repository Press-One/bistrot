...
const query = async (body, options) => {
    ...
    const resp = await fetch(`${await config()}/v2/search`, {
...
const queryBlockByBlocuNum = async (block_num, options) => {
    options = options || {};
    block_num = utilitas.ensureInt(block_num);
    utilitas.assert(block_num, 'Invalid block_num.', 400);
    const resp = await query({
        number: 1,
        option: { outputitems: !options.verifyOnly },
        query: {
            bool: {
                must: { condition: { must: [...cdtBlock, { block_num }] } },
            },
        },
    });
    utilitas.assert(resp.count, 'Block not found.', 404);
    return !!options.verifyOnly ? !!resp.count : resp.items?.[0];
};

const verifyBlockByBlocuNum = async (block_num) => {
    let resp = false;
    try { resp = await queryBlockByBlocuNum(block_num, { verifyOnly: true }); }
    catch (e) { }
    return resp;
};
...
const verifyIndex = async (options) => {
    ...
    let [id, resp] = [await lastIdGet(), []];
    let max = id + current * (options.scale || 1);
    ...
    let [lastVerifiedId, pending] = [id, []];
    for (let i = 0; i < resp.length; i++) {
        const curId = id + i + 1;
        if (resp[i] && !pending.length) {
            lastVerifiedId = curId;
        } else if (!resp[i]) {
            pending.push(curId);
        }
    }
...
const handleAction = async (action, transaction, block) => {
    const k = action?.data;
    ...
    await newAction({ action: act, transaction });
};
