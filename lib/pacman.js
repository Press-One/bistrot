...
const getShpApi = () => {
    const url = utilitas.getConfigFromStringOrArray(config.shpApi);
...
const queryStatements = async (
    eAccount, timestamp, type, count, options = {}
) => {
    account.assertName(eAccount);
    timestamp = assertTimestamp(timestamp);
    ...
    let [sqlIncome, sqlExpense, sqlCase, cdtAmt, cdtRst] = [[[
        ...
        `transactions_trx_transaction_actions_data__to_user = '${eAccount}'`,
    ], [
        ...
        `transactions_trx_transaction_actions_account = 'proxy.oracle'`,
        `transactions_trx_transaction_actions_name = 'cnfmpaymt'`,
        `transactions_trx_transaction_actions_data__to_user = '${eAccount}'`,
    ]], [[
        `transactions_trx_transaction_actions_account = 'eosio.token'`,
        `transactions_trx_transaction_actions_name = 'transfer'`,
        `transactions_trx_transaction_actions_data__from_user = '${eAccount}'`,
    ], [
        `transactions_trx_transaction_actions_account = 'prs.tproxy'`,
        `transactions_trx_transaction_actions_name = 'sync'`,
        `transactions_trx_transaction_actions_data__from_user = '${eAccount}'`,
    ...
    console.log(sqlIncome, sqlExpense);
...
const connectToShp = () => {
    disconnect();
    webSocket = new ws(getShpApi(), { perMessageDeflate: false });
    webSocket.on('message', onMessage);
};

const ensureConnecting = () => {
    if ((new Date() - lastMessageAt) / 1000 > 10) { connectToShp(); }
};
...
const init = async (latestBlockNum, blkCallback, trxCallback, options = {}) => {
    ...
    if (config.serviceStateHistoryPlugin) {
        return await (options && options.event || event).loop(
            ensureConnecting, 1, 10, 3
        );
    }
};
