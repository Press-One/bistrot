...
const buildXmlJson = (node, ext) => {
    ...
    node = Object.assign({
        ...
        [key('XMLURI')]: node[key('URI')] ?? `${node[key('URI')]}/xml`,
    }, node);
...
const packIndex = (action, transaction, block) => {
    ...
    if (xmlJson) {
        act = buildXmlJson(act, {
            ...
            TITLE: `Transaction ${act.TRANSACTION_ID}`,
            URI: `${uriRoot}transactions/${act.TRANSACTION_ID}`,
        });
        block = buildXmlJson(block, {
            CLASS: classBlack, CONTENT: null,
            TITLE: `Block ${block.block_id} (${block.block_num})`,
            URI: `${uriRoot}blocks/${block.block_id}`,
            TRANSACTION_IDS: block.transactions.map(x => { return x.trx.id }),
        });
    }
