...
const stepDefault = 1000 * 1000;
const stLimit = { min: curDefault, max: stepDefault };
...
const verifyIndex = async (options) => {
    ...
    const [vId, pending] = [id + 1, []];
    let [max, stMx] = [id + current * (options.scale || 1), id + stepLength];
    options.max && (max = Math.min(options.max, max));
    options.max && (stMx = Math.min(options.max, stMx));
    let [resp, lastVerifiedId] = [noVerify(
    ) ? false : await verifyBlockByBlocuNum([vId, stMx]), id];
    if (resp) { lastVerifiedId = stMx; } else {
        resp = [];
...
const init = async (options) => {
    ...
    stepLength = utilitas.ensureInt(options.stepLength || stepDefault, stLimit);
