...
const callbacks = {
    ...
    heartbeat: null, newIndex: null, newBlock: null,
};
...
const keys = {
    ...
    PARAMS_DATA_BILL_ID: ['params.data.bill_id', UTK],
    PARAMS_DATA_CONTENT_ID: ['params.data.content_id', UTK],
    ...
    PARAMS_DATA_WRAPPING_ID: ['params.data.wrapping_id', UTK],
...
const valuePack = (k, v) => {
    ...
    switch (rule(k)) {
        ...
        case TME: v = { [_text]: new Date(Number(v) * 1000).getTime(), ...aLNG }; break;
...
const initIdGet = async () => {
    return (curId = await runCallback(
        'initIdGet', quorum.getLastIrreversibleBlockNumber
    ));
};
...
const verifyIndex = async (a, op) => {
    return await runCallback('verifyIndex', async (id) => { return 0; }, a, op);
};
...
const newIndex = async (arg) => {
    return await runCallback('newIndex', voidFun, arg);
};

const newBlock = async (arg) => {
    return await runCallback('newBlock', async (block) => {
        return !callbacks.newIndex && !callbacks.verifyIndex && log(block);
    }, arg);
};
...
const fetchBlock = async (block_num, options) => {
    ...
    try { block = await quorum.getBlockByNumberOrHash(block_num); }
    ...
    for (let i of block?.transactions?.length ? block.transactions : [null]) {
        ...
        await (options?.newIndex || optOutput || newIndex)(handleIdx(i, block));
    }
...
const analytics = async () => {
    ...
    const lastIrreversibleId = await quorum.getLastIrreversibleBlockNumber();
