...
const packTransaction = async (transaction, options = {}) => {
    if (transaction) {
        ...
        if (transaction[amtKey]) {
            transaction[amtKey] = restoreAmount(transaction[amtKey]);
        }
    }
    return transaction;
};
...
const init = (getLatestBlockNum, blkCallback, trxCallback, options = {}) => {
    blockCallback = blkCallback;
    transactionCallback = trxCallback;
    cusGetLatestBlockNum = getLatestBlockNum;
    if (config.serviceStateHistoryPlugin && config.shpApi) {
        (async () => {
            const { event } = options && options.event
                ? { event: options.event } : require('utilitas');
            await event.loop(ensureConnecting, 1, 10, 3, loopName);
        })();
    }
};
