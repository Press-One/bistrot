...
const getApiUrl = () => {
    return isArray(config.chainApi)
        ? config.chainApi[getRandomInt(config.chainApi.length)]
        : config.chainApi;
};
...
const query = async (account, timestamp, type, count, options) => {
    ...
    if (timestamp) {
        args.push(['timestamp', timestamp.toISOString()]);
    }
    if (type) {
        args.push(['type', type]);
    }
    const req = {
        method: 'GET',
        json: true,
        uri: `${getApiUrl()}/api/chain/statements/${account}?${args.map(x => {
