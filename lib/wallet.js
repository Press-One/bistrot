...
const { utilitas, account, prsUtility } = require('sushitrain');
...
const bufToHex = (buffer) => { return bufToStr(buffer, 'hex'); };
...
const recoverPrivateKey = (password, keystore, option) => {
    ...
    return option.legacy
        ? {
            privatekey: bufToHex(privateKey),
            address: prsUtility.privateKeyToAddress(privateKey),
        } : {
