...
const getRpcUrl = async (path) => {
    ...
    if (!rpcUrl && conf.speedTest && conf.debug) {
        log('Evaluating RPC API nodes...');
    }
    rpcUrl = conf.speedTest ? await network.pickFastestHost(
        conf.rpcApi, { debug: conf.debug }
    ) : utilitas.getConfigFromStringOrArray(conf.rpcApi);
    utilitas.assert(rpcUrl, 'RPC api root has not been configured', 500);
    return `${rpcUrl}${path ? ('/v1/' + path) : ''}`;
};

const getRpc = async () => {
    return new JsonRpc(await getRpcUrl(), { fetch });
};
...
const fullLengthLog = (string, maxLength) => {
    ...
    if (string.length + 4 > maxLength) {
        const full = utilitas.makeStringByLength(pad, maxLength);
        console.log(`${full} \n${string} \n${full} `);
    } else {
        string = string ? ` ${string} ` : '';
        const lLen = Math.floor((maxLength - string.length) / 2);
        const rLen = maxLength - lLen - string.length;
        console.log(`${utilitas.makeStringByLength(pad,
            lLen)}${string}${utilitas.makeStringByLength(pad, rLen)}`);
    }
