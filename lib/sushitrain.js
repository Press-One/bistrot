...
const getPreparedActor = (account) => {
    const result = config.eosAccounts && config.eosAccounts[account] && {
        account: account,
        privateKey: config.eosAccounts[account].privateKey,
        publicKey: config.eosAccounts[account].publicKey,
    };
    utilitas.assert(
...
const preparedTransact = async (actor, ctAccount, name, data, options = {}) => {
    const { account, privateKey } = getActorConfig(actor);
