...
const throwRpcError = (error, status = 500, options = {}) => {
    if (!options.ignoreError || !options.ignoreError.test(error)) {
        ...
        if (config.debug && (options.logs || err.json)) {
            ...
            if (err.json) {
                output.push(
                    '', 'RPC Error:', '', utilitas.prettyJson(err.json), ''
                );
            }
...
const rpcRequest = async (method, api, args, options = {}) => {
    args = args || {};
    const resp = await request({
        method: String(method || '').toUpperCase(),
        uri: getRpcUrl(api), json: true, args, ...options
    }).promise();
...
const fullLengthLog = (string, maxLength) => {
    ...
    if (string.length + 4 > maxLength) {
        ...
        console.log(`${makeStringByLength(pad, lLen)} ${string} `
