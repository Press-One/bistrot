...
const getRpcUrl = async (path, options) => {
    ...
    utilitas.assert(
        options.rpcapi || rpc, 'RPC api root has not been configured', 500
    );
    return `${options.rpcapi || rpc}${path ? ('/v1/' + path) : ''}`;
};
...
const getClient = async (account, privateKey, options) => {
    options = options || {};
    const key = !account && !privateKey ? '_' : `${account}_${privateKey}`;
    if (key && clients[key]) {
        return clients[key];
    }
...
const throwRpcError = async (error, status = 500, options = {}) => {
    if (!options.ignoreError || !options.ignoreError.test(error)) {
        ...
        if ((await config()).debug && (options.logs || error.json)) {
            const output = [];
            if (options.logs) {
...
const rpcRequest = async (method, api, body, options) => {
    ...
    utilitas.assert(resp && !resp.error, [
        `Error querying RPC api:`, resp,
        `${(body ? `echo '${body}' | ` : '')}http ${method} ${url}`,
    ].join('\n'), 500);
    return resp;
};
...
const getBlockByNumOrId = async (blockNumOrId, options) => {
    utilitas.assert(blockNumOrId, 'Invalid block num or id.', 400);
    const resp = await rpcRequest(
        'POST', 'chain/get_block', { block_num_or_id: blockNumOrId }, options
    );
    utilitas.assert(
        resp?.id,
        `Error fetching block info (${blockNumOrId}): ${JSON.stringify(resp)}.`,
        500
    );
    return resp;
};
...
const getTransactionById = async (id, block_num_hint = 0) => {
    ...
    utilitas.assert(resp?.id, 'Error fetching transaction info.', 500);
    return resp;
};
...
const getLastIrreversibleBlockNumber = async (args) => {
    return (await getInfo(args))?.last_irreversible_block_num;
};
