...
const queryTransactionByLegacyId = async (id, options) => {
...
const queryTransactionById = async (id, options) => {
    id = String(id).trim().toLowerCase();
...
const queryBlockNumByTransactionId = async (id, options) => {
    options = { ...options || {}, raw: true };
    return (await queryTransactionById(id, options))?.[pacman.key('BLOCK_NUM')];
};
...
module.exports = {
    ...
    queryBlockNumByTransactionId,
    queryLastBlock,
    queryLastBlockNum,
    queryTransactionById,
    queryTransactionByLegacyId,
    sortByBlockNum,
    sortByBlockNumDesc,
