...
const decodeLogs = async (logs, options) => {
    const abis = await getAllAbis(options);
    ...
    for (let i in logs || []) {
        ...
        for (let name in abis) {
            ...
            try {
                const _rs = iface.parseLog(logs[i]);
                _rs && (rs = {
                    name: _rs.name,
                    events: _rs.args.map((v, i) => ({
                        name: _rs.fragment.inputs[i].name,
                        type: _rs.fragment.inputs[i].type,
                        value: v,
                    })),
