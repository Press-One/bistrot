...
const initContract = async (abi, address, options) => {
    let [client, contract] = [await getEthProvider(options), null];
    if (address) {
        contract = new ethers.Contract(address, abi, client);
    } else {
...
const executePreparedContractMethod = async (abi, method, arg, opts) => {
    opts?.privateKey && assertString(opts?.privateKey, 'Invalid privateKey.');
    // console.log(opts?.privateKey);
    // const wallet = new BaseWallet(opts?.privateKey);
    // console.log(wallet);
    const callOpts = {
        ...opts?.privateKey ? { from: privateKeyToAddress(opts?.privateKey, { standard: true }) } : {},
        ...opts?.call || {},
    };
    const { client, contract } = await initPreparedContract(abi, opts);
    // patched nonce too low error by @LeaskH
    if (!callOpts.nonce && callOpts?.from) {
        callOpts.nonce = await client.getTransactionCount(callOpts?.from);
    }
    assert(contract?.[method], `Invalid method: '${method}'.`, 400);
    let resp = await contract[method](...arg || []);
