...
const assertAddress = (a, e, c) => {
    ...
    utilitas.assert(web3.utils.isAddress(a), e || 'Invalid address.', c || 400);
    return web3.utils.toChecksumAddress(a);
};

const assertHash = (h, e, c) => {
    h = utilitas.ensureString(h);
    lib.utilitas.assert(h.length === 66, e || 'Invalid transaction hash.', c || 400);
    return h;
};
