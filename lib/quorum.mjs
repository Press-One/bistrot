...
const assertAddress = (address, error, code) => {
    address = utilitas.ensureString(address);
    utilitas.assert(
        address.length === 42 && web3.utils.isAddress(address),
        error || 'Invalid address.', code || 400
    );
    return web3.utils.toChecksumAddress(address);
};

const assertHash = (hash, error, code) => {
    hash = utilitas.ensureString(hash);
    lib.utilitas.assert(
        hash.length === 66, error || 'Invalid transaction hash.', code || 400
    );
    return hash;
};
