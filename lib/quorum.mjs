...
import { network, utilitas } from 'utilitas';
...
import {
    getAbiByNameOrAddress, getAbiDecoder, getContractNameByAddress,
    getFileByName, getSolByName,
} from './etc.mjs';
...
const getRpcUrl = async (options) => {
    ...
    if (!options.rpcapi && !rpc) {
        rpc = utilitas.getItemFromStringOrArray((await config()).rpcApi);
    }
...
const getClient = async (privateKey, options) => {
    options = options || {};
    const key = privateKey ? privateKeyToAddress(privateKey) : '_';
...
const executePreparedContractMethod = async (abi, method, act, arg, opts) => {
    ...
    const callOpts = {
        ...opts?.privateKey ? { from: privateKeyToAddress(opts?.privateKey) } : {},
        ...opts?.call || {},
    };
...
const deployContract = async (sol, args, opts) => {
    ...
    const from = opts?.privateKey ? {
        from: privateKeyToAddress(opts?.privateKey),
    } : {};
    ...
    return await instant.send({
        ...from, gas: utilitas.ensureInt(await instant.estimateGas()) + 1,
    });
};

const deployPreparedContract = async (contractName, args, options) => {
    const sol = await getSolByName(contractName, {});
