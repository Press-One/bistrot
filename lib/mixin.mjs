...
const completeAssetIcon = (icon) => {
    return icon.startsWith('http') ? icon : `https://mixin-images.zeromesh.net/${icon}=s128`;
};
...
const queryAssetOrChain = (id, lib, index) => {
    ...
    if (utilitas.isSet(index)) {
        ...
        assert((id = String(id || '')), 'Invalid name or id.', 400);
...
const verifyPaymentArgs = (
    ...
    assert(utilitas.verifyUuid(trace), 'Invalid trace.', 400);
