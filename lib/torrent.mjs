...
const pmfCreateTorrent = promisify(cbfCreateTorrent); // @TODO: debug remove api
...
const getStreamingAddress = (subPath) => join(
    `http://localhost:${_client._server.server.address().port}`,
    pathname, subPath || ''
);
...
const render = (status) => {
    ...
    status.torrents.map(r => {
        ...
        for (let k in t) {
            ...
            const available = process.stdout.columns - _maxKeyLength - 3;
            const v = Array.isArray(t[k]) ? (t[k].length ? t[k] : ['']) : [t[k]];
            for (let i in v) {
                const [sV, more] = [String(v[i]), ~~i > 2];
                console.log([
                    (i === '0' ? k : '').padEnd(_maxKeyLength, ' '),
                    (more && `*   ${v.length - 3} more item(s)...`)
                    || (sV.length > available && `${sV.substring(0, available - 3)}...`)
                    || sV,
                ].join(' : '));
                if (more) { break; }
            }
