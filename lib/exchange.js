...
const swapToken = async (
    ...
    if (options.reverse) {
        amount = finance.parseAndFormat(math.divide(
            math.divide(amount, rate), math.subtract(1, catchPool.swap_fee)
        ));
    }
    ...
    if (!options.dryrun) {
        ...
        result.payment_request = await getPaymentRequest(chainAccount);
    }
    return result;
};
...
const rmLiquid = async (
    ...
    options = options || {};
    options.chainAccountRequired = !options.dryrun;
    options.mixinAccountRequired = !options.dryrun;
    const bMixinAccount = options.mixinAccount || utilitas.extract(
        options.dryrun ? null : await atm.queryMixinBoundByAccount(
            chainAccount, { assert: true }
        ), 'bound_account');
    var {
    ...
    if (!options.dryrun) {
        await account.ensureAuth(chainAccount, null, privateKey);
        result.transaction = await sushitrain.transact(
...
const getPaymentRequest = async (chainAccount) => {
    ...
    let [tr, ts] = await Promise.all([swap.getAllTrxs(), swap.getAllTrans()]);
    let [trxs, trans] = [{}, []];
    tr.filter(x => {
    ...
    for (let x of ts) {
        ...
        for (let j in x.payment_request || {}) {
            x.payment_request[j].payment_url = await system.magicPayment(
                x.payment_request[j].payment_url, { screening: true }
            );
        }
        trans.push(x);
    }
