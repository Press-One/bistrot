...
const addLiquid = async (
    ...
    currency_b, amount_a, email, memo, options
) => {
    options = options || {};
    options.chainAccountRequired = true;
    var {
...
const rmLiquid = async (
    privateKey, chainAccount, currency_a, currency_b, amount, email, memo, opts
) => {
    opts = opts || {};
    opts.chainAccountRequired = true;
    var { chainAccount, email, currency: currency_a, amount, requestId, memo }
        = mixin.verifyPaymentArgs(
            chainAccount, null, null, email, currency_a,
            amount, null, memo || swap.rmLiquidMemo, opts
        );
    currency_b = utilitas.trim(currency_b, { case: 'UP' });
    const mixin_account_id = email ? { email } : {};
    const catchPool = await swap.getPoolByCurrency(currency_a, currency_b);
