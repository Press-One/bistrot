...
cleos--url http://51.255.133.170:8888 push action prs.swap rmliquid '[123456, "testuser1", "ta.tb", "1000000.0000 TATB", "mixin_tid", "mixni_aid", "memo"]' -p testuser1@active

const rmLiquid = async (
    privateKey, chainAccount, pool_name, amount, email, memo, options
) => {
    options = options || {};
    options.chainAccountRequired = true;
    var { chainAccount, email, amount, requestId, memo }
        = mixin.verifyPaymentArgs(
            chainAccount, null, null, email, currency_a, amount_a,
            null, memo || swap.rmLiquidMemo, options
        );
    currency_b = utilitas.trim(currency_b, { case: 'UP' });
    const cidB = mixin.assetIds[currency_b] && mixin.assetIds[currency_b].id;
    const mixin_account_id = email ? { email } : {};
