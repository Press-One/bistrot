...
const mint = (add, token, key, options) => sendToPreparedMethod(
    add, 'mint', [assertAsset(token)], key, options
);

const balanceOf = async (a, ownr, o) => Number(
    await callPreparedMethod(a, 'balanceOf', [ownr], o)
);
