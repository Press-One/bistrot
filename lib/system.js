...
const requestSushichef = async (method, path, urlArgs, body, error, option) => {
    option = option || {};
    global.chainConfig = { chainApi: 'http://ssss.com' };
    console.log('adasd', await sushitrain.config({ basicConfigOnly: true, authority: true }));
    return;
    console.log(await sushitrain.config({ basicConfigOnly: true }));
