...
const getEthClient = async (opts) => { return (await getClient(opts)).eth; };
...
const getClient = async (options) => {
    options = options || {};
    return client || (
        client = new web3(web3.givenProvider || await getRpcUrl(null, options))
    );
};

const getBlock = async (numOrHash, options) => {
    return await (await getEthClient(options)).getBlock(numOrHash, true);
};

const getTransaction = async (hash, options) => {
    return await (await getEthClient(options)).getTransaction(hash);
};

const getBalance = async (address, options) => {
    return await (await getEthClient(options)).getBalance(address);
};
