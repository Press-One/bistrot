...
const executePreparedContractMethod = async (key, abi, method, act, arg, o) => {
    assertString(key, 'Invalid privateKey.');
    const from = crypto.privateKeyToAddress(key);
    const pc = await initPreparedContract(abi, { ...o || {}, privateKey: key });
    utilitas.assert(pc.methods?.[method], `Invalid method: '${method}'.`, 400);
    utilitas.assert(acts.has(act), `Invalid action: '${act}'.`, 400);
    return await pc.methods[method].apply(null, arg ?? [])[act]({ from });
};

const callPreparedContractMethod = async (key, abi, mth, arg, o) => {
    return await executePreparedContractMethod(key, abi, mth, actCall, arg, o);
};

const sendToPreparedContractMethod = async (key, abi, mth, arg, o) => {
    return await executePreparedContractMethod(key, abi, mth, actSend, arg, o);
};
