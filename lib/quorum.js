...
const getContractNameByAddress = async (address, options) => {
    return (await etc.getAbiByAddress(address, options)).contract;
};
...
const packTransaction = async (trx, receipt, options) => {
    ...
    return {
        ...trx, ...options?.raw ? {} : {
            contract: await getContractNameByAddress(trx.to, options),
            ...await decodeMethod(trx.input)
        }, ...receipt ? { receipt } : {},
    };
};
...
const getBalanceByAddress = async (address, options) => {
    return await (await getEthClient(null, options)).getBalance(address);
...
const initPreparedContract = async (name, options) => {
    const { abi, address } = await etc.getAbiByName(name, options);
...
const getInfo = async (options) => {
    const client = await getEthClient(null, options);
    const values = [
        'defaultAccount',
    ...
    const resp = {};
    values.map(x => { resp[x] = client[x]; });
    (await batchRequest(toFetch)).map((x, i) => {
        const key = toFetch[i].split('.').pop().replace(/^get/i, '');
