...
const compile = (content, options) => {
    ...
    const imports = options?.import || ((pth) => {
        let abs, contents;
        if ((abs = pth && pth.replace(/^(@openzeppelin)/, 'node_modules/$1'))) {
            try {
                utilitas.assert(!options?.depOnly, "Wolf's Coming!", 200);
    ...
    let [resp, errMsg] = [
        JSON.parse(solc.compile(JSON.stringify(code), { import: imports })),
        'Error compiling contract source code.', null
    ];
    const contracts = resp?.contracts?.[defaultSol];
    const defaultKy = Object.keys(contracts || {})[0];
    if (resp?.errors?.['0'] || !defaultKy) {
        errMsg = resp?.errors?.['0'].message || errMsg;
        options?.assert ? utilitas.throwError(errMsg, 400) : log(errMsg);
    }
    if (options?.depOnly) { return dependencies; }
    else if (options?.raw) { return resp; }
