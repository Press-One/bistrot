...
const decodeLogs = async (logs, options) => {
    ...
    for (let i in logs || []) {
        const events = {};
        for (let j of resp[i].events) { events[j.name] = j.value; }
        resp[i].events = events;
        logs[i] = {
            ...logs[i], ...resp[i], contract: (
                await etc.getAbiByAddress(logs[i].address, options)
            ).contract,
        }
    }
    return logs;
...
const getTransactionReceiptByHash = async (hash, options) => {
    const resp = await (await getEthClient(null, options)).getTransactionReceipt(hash);
    await decodeLogs(resp.logs);
    return resp;
};
