...
const transfer = async (from, privateKey, to, qnt, memo, options) => {
    utilitas.assert(qnt = parseAndFormat(qnt), 'Invalid quantity.', 400);
    return await sushitrain.transact(
        from, privateKey, 'eosio.token', 'transfer',
        { from, to, quantity: qnt, memo: memo || defaultTransMemo }, options
    );
};
