...
const bigFormat = (bignumber, currency) => {
    return mathjs.format(bignumber || 0, {
...
const bignumberSum = function() {
    return bigFormat(mathjs.add.apply(null, [
        0, ...Array.from(arguments).map((x) => { return mathjs.bignumber(x); })
    ]));
};
...
const { utilitas, mathjs } = require('utilitas');
