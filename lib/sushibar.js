...
const getNodes = async () => { return await requestApi('GET', 'nodes'); };
...
const getApiUrl = async (path, args) => {
    const c = await config({
        ...
        authority: utilitas.insensitiveCompare(path, 'nodes')
    });
    if (!api) {
        if (c.speedTest) {
            if (c.debug) { log('Evaluating Chain API nodes...'); }
            api = await network.pickFastestHost(c.chainApi, { debug: c.debug });
        } else { api = utilitas.getConfigFromStringOrArray(c.chainApi); }
    }
    utilitas.assert(api, 'Chain api root has not been configured', 500);
    return path ? utilitas.assembleApiUrl(api, `api/${path}`, args) : api;
};
