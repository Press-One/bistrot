...
const assembleChainApiUrl = async (path, args) => {
    return assembleApiUrl(await getChainApiUrl(), `api/${path}`, args);
};
...
const getTransactionById = async (transactionId) => {
    ...
    const resp = await requestChainApi(
        'GET', `chain/transactions/${transactionId}`,
        null, null, 'Error querying transaction.'
    );
    utilitas.assert(
        resp
        && resp.data
    ...
    utilitas.assert(
        ...
        && trx.actions.length
        && trx.actions[0]
        && trx.actions[0].data, 'Invalid transaction data.', 500
    );
