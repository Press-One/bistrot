...
const getTransactionById = async (trxId) => {
    trxId = String(trxId || '').trim().toUpperCase();
    assert(trxId, 'Invalid transaction id.');
    let [trx, error] = [null, 'Error querying transaction.'];
    const resp = await requestChainApi(
        'GET', `chain/transactions/${trxId}`, null, null, error
    );
    assert(
        resp
        && resp.data
    ...
    return { block: trx.actions[0].data, transaction: resp.data };
};
