...
const assertNotPreservedName = async (
    ...
    ]).includes(utilitas.ensureString(name, { case: 'LOW' })), error, status);
};
...
const linkClaimrewardAuth = async (actor, privateKey, options) => {
    return await linkAuth('claimreward', 'claimer', actor, privateKey, options);
};

const unlinkClaimrewardAuth = async (actor, privateKey, options) => {
    return await unlinkAuth('claimreward', actor, privateKey, options);
};
