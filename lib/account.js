...
const buyResource = async (
    actor, receiver, type, quant, privateKey, options = {}
) => {
    receiver ? assertName(receiver, 'Invalid receiver name.')
        : (receiver = actor);
    utilitas.assert(
        ['buyram', 'buynet'].includes(type), 'Invalid resource type.', 400
    );
    return await sushitrain.transact(actor, privateKey, 'eosio', type, {
        payer: actor, receiver: receiver, quant: finance.formatAmount(quant),
    }, options);
};

const buyRam = async (actor, receiver, quant, privateKey, options = {}) => {
    return await buyResource(
        actor, receiver, 'buyram', quant, privateKey, options
    );
};

const buyNet = async (actor, receiver, quant, privateKey, options = {}) => {
    return await buyResource(
        actor, receiver, 'buynet', quant, privateKey, options
    );
};
