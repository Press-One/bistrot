...
const { account, mixin, sushitrain, finance, uuid, utilitas, crypto }
    = require('sushitrain');
...
const updateAuth = async (acc, publicKey, privateKey, options = {}) => {
    const [
        prsc_and_tproxy_auth_transaction, swap_auth_transaction
    ] = await Promise.all([
        account.updateAuth('active', publicKey, [
            makePermission('prs.prsc', 'eosio.code'),
            makePermission('prs.tproxy', 'eosio.code'),
        ], acc, privateKey, options),
        account.updateAuth('active', publicKey, [
            makePermission('prs.swap', 'eosio.code'),
        ], acc, privateKey, options),
    ]);
    return { prsc_and_tproxy_auth_transaction, swap_auth_transaction };
};
...
const requestPayment = async (
    ...
    var {
        ...
        currency, amount, requestId, memo
    ...
    return {
        chainAccount, mixinAccount, mixinId, email, currency,
