...
const getPaymentRequest = async (chainAccount, options) => {
    ...
    resp = {
        ...
        transaction_id: resp.trx_id_req,
        type: finance.getTransTypeByValue(resp.type),
        status: finance.getTransStatusByValue(resp.status),
        createdAt: new Date(parseInt(resp.time_stamp_req) * 1000).toISOString(),
    };
    return await packDeposit(resp, options);
};
