...
const makeActions = (account, name, data, actor) => {
    ...
    let [arrAc, arrNm, arrDt, actions] = [
        utility.isArray(account), utility.isArray(name), utility.isArray(data),
        []
    ];
    assert(arrAc === arrNm === arrDt, 'Invalid transaction parameters.');
...
const verifyPaymentArgs = (
    ...
    assert(
        ...
        || (utility.verifyUuid(mixinAccount) || verifyMxinId(mixinId)),
        'Invalid Mixin account');
...
const transact = async (actor, privateKey, account, name, data, options) => {
    ...
    // console.log(utility.json(actions));
    try {
