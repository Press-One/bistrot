...
const bigFormat = (bignumber) => {
    return mathjs.format(mathjs.bignumber(bignumber), {
        notation: 'fixed',
        precision: 4,
    }).replace(/"/g, '');
};
...
const makeActions = (account, name, data, actor) => {
    ...
    assert(
        arrAc === arrNm && arrNm === arrDt, 'Invalid transaction parameters.'
    );
...
const verifyPaymentArgs = (
    ...
    assert(
        !options.mixinAccountRequired || utilitas.verifyUuid(mixinAccount),
        'Invalid Mixin account.');
    assert(
        !options.mixinAccountOrIdRequired
        || (utilitas.verifyUuid(mixinAccount) || verifyMxinId(mixinId)),
        'Invalid Mixin account');
    assert(!email || utilitas.verifyEmail(email), 'Invalid email.');
    assert(currencyId, 'Invalid currency.');
    assert(amount, 'Invalid amount.');
    assert(utilitas.verifyUuid(trace), 'Invalid trace.');
    return {
        chainAccount, mixinAccount, mixinId, email,
...
const transact = async (actor, privateKey, account, name, data, options) => {
    ...
    let [actions, result] = [makeActions(account, name, data, actor), null];
...
const queryProducersByRange = async (lower_bound, limit) => {
    const [result, resp] = [[], await defaultClient().api.rpc.get_producers(
