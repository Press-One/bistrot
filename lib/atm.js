...
const {
    account, mixin, sushitrain, finance,
    uuid, utilitas, crypto, sushibar, fetch
} = require('sushitrain');
...
const getAccount = async (acc) => {
    const [accResp, accBound] = await Promise.all([
        account.getByName(acc), queryMixinBoundByAccount(acc)
    ]);
    utilitas.assert(accResp, `Account Not Found (${acc}).`, 404);
    return Object.assign(accResp, {
        bound_mixin_account: utilitas.extract(accBound, 'bound_account'),
    });
};
...
const bindIdentity = async (acc, privateKey, options = {}) => {
...
const queryMixinBoundByAccount = async (acc, options) => {
    ...
    return { transaction, bound_account };
};

const getAuthUrl = (api) => {

};

const rewardAuth = async (acc, privateKey, options = {}) => {
    const resp = await sushitrain.addClaimerAuth(acc, privateKey);
    const data = { keystore: resp.keystore, account: acc };
    const signature = crypto.signData(data).signature;
    fetch('')
};
