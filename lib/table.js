...
const query = async (account, table, options = {}) => {
    ...
    let rs = await sushitrain.rpcRequest('POST', 'chain/get_table_rows', query);
    utilitas.assert(rs && !rs.error, rs && rs.message
        ? rs.message : `Error fetching data from '${account}->${table}'.`, 500);
    return rs;
};
