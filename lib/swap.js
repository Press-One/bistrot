...
const [contract, defaultTransMemo, addLiquidMemo, rmLiquidMemo, plErr] = [
    'prs.swap', 'Swap tokens via PRESS.one', 'Add swap liquid to PRESS.one',
    'Remove swap liquid from PRESS.one', 'No pool is available to swap'
];
...
const fT = async (t, f) => { return (await table.getAll(contract, t)).map(f); };
const getAllPools = async () => { return await fT('pool', packPool); };
const getAllTrxs = async () => { return await fT('trxs', packTrx); };
const getAllTrans = async () => { return await fT('trans', packTran); };
