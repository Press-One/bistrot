...
const payReqType = [null, 'SWAP', 'ADD_LIQUID', 'RM_LIQUID'];
const payReqStatus = [
    null, 'WAIT_COLLECT', 'WAIT_SETTLE', 'WAIT_ORACLE',
    'SUCCESS', 'FAILED', 'TIMEOUT'
];

const getEnum = (enumArr, enumInt) => { return enumArr[enumInt] || 'UNKNOW'; };
const getPoolStatus = (enumInt) => { return getEnum(poolStatus, enumInt); };
const getPayReqType = (enumInt) => { return getEnum(payReqType, enumInt); };
const getPayReqStatus = (enumInt) => { return getEnum(payReqStatus, enumInt); };
...
const packTrans = (p) => {
    p.type = getPayReqType(p.type);
    p.status = getPayReqStatus(p.status);
    return p;
};
...
const getAllTrxs = async () => {
    return await table.getAll('prs.swap', 'trxs');
};

const getAllTrans = async () => {
    const resp = await table.getAll('swap.oracle', 'trans');
    return resp.map(packTrans);
};
