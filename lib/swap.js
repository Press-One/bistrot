...
const packTran = (p) => {
    try { p.mixin_trace_id = JSON.parse(p.mixin_trace_id); } catch {
        p.mixin_trace_id = [p.mixin_trace_id, p.mixin_trace_id];
    }
        ...
        p.payment_request[p.mixin_trace_id[pI]] = {
            ...
            payment_url: p.type === 'COLLECT'
                ? mixin.createPaymentUrlToOfficialAccount(
                    plan[0], p.mixin_trace_id[pI], p.memo, { currency: plan[1] }
                ) : null,
        };
    }
    return p;
