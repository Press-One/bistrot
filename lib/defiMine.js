...
const standardize = (price, rate) => {
    ...
    return mathjs.format(mathjs.divide(
        mathjs.bignumber(price), mathjs.bignumber(rate)
...
const fetchPriceFromBinance = async (coins, base = 'USDT', options = {}) => {
    ...
    const [batch, result, url] = [
        Array.isArray(coins), {},
        utilitas.assembleApiUrl('https://api.binance.com', 'api/v3/ticker/price'),
    ];
    coins = (batch ? coins : [coins]).map(x => x.toUpperCase());
    const resp = await fetch(url).then(res => res.json());
...
const process = async () => {
    log(`Fetching prices from ${Object.keys(brokerage).join(', ')} @ ${new Date(
    ).toISOString()}...`);
...
const init = async (acc, pubKey, pvtKey, options = {}) => {
    ...
    if (config.serviceDefiMine) {
        return await (options && options.event || event).loop(
            process, submitInterval, 60 * 10, 1, null, options
        );
    }
};
