...
const transfer = async (privateKey, to, amount, options) => {
    ...
    const client = await buildClient(privateKey, options);
    ...
    options.nonce = options.nonce || await client.getTransactionCount(from);
    return await client.eth.sendTransaction(
        { from, to, value: web3.utils.toWei(amount) }, options
    );
};
