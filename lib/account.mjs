import { assertRecipient, privateKeyToAddress } from './crypto.mjs';
...
const getBalanceByAddress = async (address, opts) => {
    ...
    if (opts?.etherOnly) { return resp; }
    let [assets, keys, resps, result] = [getMirroredAsset(), [], [], {
        'ETHER': opts?.amountOnly ? resp : {
            index: 0,
            id: '00000000-0000-0000-0000-000000000000',
            name: 'RumSystem.net ETHER',
...
const transfer = async (privateKey, to, amount, options) => {
    ...
    const b = math.bignumber(await getBalanceByAddress(from, { etherOnly: 1 }));
    ...
    return await (await buildClient(privateKey, options)).eth.sendTransaction({
        from, to, value: web3.utils.toWei(amount)
    }, options);
};
