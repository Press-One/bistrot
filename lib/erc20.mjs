...
import {
    executePreparedContractMethod, deployPreparedContract,
    sendToPreparedContractMethod,
} from './quorum.mjs';

const [abiName, defaultCap] = ['RumERC20', '21000000'];
const [rrum, abiRrum] = [true, 'RRUM'];
const fromWei = (str) => convert('formatEther', str);
const toWei = (str) => convert('parseEther', str);
...
const convert = (mth, str) => ethers[mth](utilitas.ensureString(str));

const assertAmount = (amount, options) => {
    amount = _assertAmount(amount);
    return toWei(amount, options);
};
...
const deploy = async (symbol, privateKey, options) => {
    ...
    return await deployPreparedContract(options?.rrum ? abiRrum : abiName, [
        ...
        toWei(options?.cap || defaultCap, options),
...
const callPreparedMethod = (add, mth, args, opts) => executePreparedContractMethod(
