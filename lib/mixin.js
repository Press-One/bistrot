...
const defaultAssetId = currencies[defaultCurrency];
...
const verifyPaymentArgs = (
    ...
    if (options.mixinAccountRequired) {
        utilitas.assert(
            utilitas.verifyUuid(mixinAccount), 'Invalid Mixin account', errMxId
        );
    }
    if (options.mixinAccountOrIdRequired) {
        utilitas.assert(
            utilitas.verifyUuid(mixinAccount) || verifyId(mixinId), errMxId
        );
    }
    utilitas.assert(!email || utilitas.verifyEmail(email), 'Invalid email.');
    utilitas.assert(currencyId, 'Invalid currency.');
    utilitas.assert(amount, 'Invalid amount.');
    utilitas.assert(utilitas.verifyUuid(trace), 'Invalid trace.');
    return {
        chainAccount, mixinAccount, mixinId, email,
