...
const verifyPaymentArgs = (
    ...
    const currencyId = assetIds[currency].id;
    mixinAccount = String(mixinAccount || '').trim();
    mixinId = String(mixinId || '').trim();
    email = String(email || '').trim();
    amount = finance.parseAndFormat(amount);
    trace = String(trace || '').trim() || uuid.v1();
    memo = utilitas.isObject((memo = memo || finance.defaultTransMemo))
    ...
    return {
        ...
        amount, trace, requestId: getTimestampFromUuid(trace), memo,
    };
};
