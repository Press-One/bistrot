...
const officialUrlNamespace = 'https://press.one/';
const paymentUrlRoot = 'https://mixin.one/pay';
const officialAccountId = 'beb05804-f083-498e-ac0f-af6d7fbcd694';
const defaultCurrency = 'PRS'; // config.debug ? 'CNB' : 'PRS'; @todo: => RUM
const defaultAssetId = assetIds[defaultCurrency].id;

const verifyId = (mixinId) => { return /^[0-9]{1,}$/.test(mixinId); };

const completeAssetIcon = (icon) => {
    return `https://mixin-images.zeromesh.net/${icon}=s128`;
};

const getRfcUrlNamespaceUuid = () => {
    return uoid.getRfcUrlNamespaceUuid(officialUrlNamespace);
};
...
const queryAssetOrChain = (id, lib, index) => {
    ...
    if (utilitas.isSet(index)) {
        ...
        utilitas.assert((id = String(id || '')), 'Invalid name or id.', 400);
        if (utilitas.verifyUuid(id)) {
    ...
    for (let i in lib) {
        switch (type) {
            case 'INDEX':
                catched = index === lib[i].index;
                break;
            case 'UUID':
                catched = id === lib[i].id.toLowerCase();
                break;
            case 'SYMBOL':
                catched = id === i.toUpperCase();
        }
        if (catched) {
            result = lib[i];
...
const formatAmount = (number) => {
    return finance.formatAmount(number, defaultCurrency);
};

const createPaymentUrl = (
    mixinAccount, currency, amount, trace, memo, options = {}
) => {
    options.chainAccountRequired = false;
...
const verifyPaymentArgs = (
    chainAccount, strMxAccount, strMixinId, strEmail,
    currency, amount, trace, memo, opt = {}
) => {
    chainAccount = utilitas.trim(chainAccount);
    ...
    memo = utilitas.isObject(memo) ? JSON.stringify(memo) : memo;
    const mixinAccount = utilitas.verifyUuid(strMxAccount) ? strMxAccount : '';
    ...
    opt.chainAccountRequired && account.assertName(chainAccount);
    utilitas.assert(
        (opt.mixinAccountRequired || strMxAccount ? mixinAccount : true)
        && (strMixinId ? mixinId : true)
        && (opt.mixinAccountOrIdRequired ? mixinAccount || mixinId : true),
        'Invalid Mixin account.', 400
    );
    strEmail && utilitas.assert(email, 'Invalid email.', 400);
    utilitas.assert(opt.ignoreCurrency || currencyId, 'Invalid currency.', 400);
    utilitas.assert(amount, 'Invalid amount.', 400);
    utilitas.assert(utilitas.verifyUuid(trace), 'Invalid trace.', 400);
    utilitas.assert(memo.length <= 140, 'Memo too long.', 400);
    return {
        chainAccount, mixinAccount, mixinId, email, currency, currencyId,
        amount, trace, requestId: uoid.getTimestampFromUuid(trace), memo,
    };
};

const getTraceByTransactionId = (transactionId, rotate) => {
    transactionId = utilitas.rotate(transactionId, rotate, { case: 'UP' });
    utilitas.assert(transactionId, 'Invalid transaction id.', 400);
    return uuid.v5(transactionId, getRfcUrlNamespaceUuid());
};

const assetAssetSymbol = (s) => {
    s = utilitas.ensureString(s, { case: 'UP' });
    utilitas.assert(
        assetIds[s] && assetIds[s].id,
...
const maskPaymentUrl = async (url) => {
    return url.replace(paymentUrlRoot, await sushibar.getApiUrl('finance/pay'));
};
...
const { utilitas, uuid, uoid } = require('utilitas');
