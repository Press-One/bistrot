...
const readFile = async (filename) => {
    return await fs.readFile(path.join(
        path.dirname(module.filename), '..', 'etc', filename
    ), 'utf8');
};
...
const buildConfig = async (
    ...
    let [content, nodes] = await Promise.all([
        readFile('config.ini'), sushibar.getNodes()
    ]);
    const adds = [];
    nodes.map(x => {
        if (x.status && x.p2p_port) { adds.push(`p2p-peer-address = ${x.ip}`); }
    });
    content = content.replace(/\{\{p2p\-peer\-address\}\}/g, adds.join('\n'));
