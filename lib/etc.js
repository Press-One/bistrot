...
const getProtoByName = async (name, type, op) => {
    let proto = getFileByName(`proto${name}.json`, { ...op || {}, json: true });
    if (!op?.raw) {
        proto = protobuf.Root.fromJSON(proto);
        if (type) { proto = proto.lookupType(type); }
    }
    return proto;
};
