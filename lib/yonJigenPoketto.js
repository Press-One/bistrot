    ...
    const blocks = separate.map(x => {
        const block = {
            ...
            meta: {
                ...
                price_per_bytes_ms: pricePerBytesMs,
                size: unitSize,
            },
            data: {
                currency: 'PRS',
...
const getSliceFromPeer = async (ipfsCid, ipfsPeerId) => {
    ...
    try {
        ...
        assert(false, 'Failed to get data slice from IPFS peer: .');
    }
...
const sliceVericationHash = (data, publicKey) => {
    ...
    return prsUtility.keccak256(prsUtility.keccak256(data) + publicKey);
};
...
const challengePromise = async (transactionId, publicKey, privateKey) => {
    ...
    const slice = pms.block.data.slices[
        utility.getRandomInt(pms.block.data.slices.length)
    ];
    const buf = await getSliceFromPeer(slice, pms.block.data.ipfs_peer_id);
    assert(buf, `Invalid storage: ${slice}@${pms.block.data.ipfs_peer_id}.`);
    const now = new Date();
    const block = {
        id: uuid(),
        user_address: publicKey,
        ...
        data: {
            ...
            uri: slice,
            result: sliceVericationHash(buf, publicKey),
...
(async () => {
    try {
        ...
        //     '5997f594cbe707c115c82deada614fa657e34d47c962778dc6c37f02809eb88d'
        // );
        // console.log(gResult);
