...
const requestStore = async (
    ...
    time = defaultStorageTime,
    options = {}
) => {
    assert(
        (time = ~~time) >= defaultStorageTime,
    ...
    const objSep = {};
    separate.map(x => { objSep[`${transactionId}:${x.join(',')}`] = x; });
    const price = pricePerBytesMs * time * trxFml.formula.size;
    const block = {
        id: uuid(),
        user_address: publicKey,
        type: 'PUBLISH:3',
        meta: {
            univalent: price / share,
        },
        data: {
            transactionId,
            price,
    ...
    return { fileBlock: trxFml.block, formula: trxFml.formula, block, result };
};
...
(async () => {
    try {
        ...
        // const rResult = await requestStore(
        //     'f4a242b5401ddfb6eeccb303720bf640d8543c9c7a51fd2e0c60b878f9ea1894',
        //     'EOS7ZtBTNsekNJWJHMieEeELQWPpVwbKs7ezVE9nKk9rpkJ69wxUR',
        //     '5Jyqyqzx1FPzU6EqtmG4vLUABTuNR6P59eeSBW6an2rStuEgj77',
        // );
