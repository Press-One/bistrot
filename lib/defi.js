...
const { sushitrain, table } = require('sushitrain');
...
const checkPrice = async () => {
    ...
    resp = resp.map(x => {
        utilitas.assert(
            x.prices && x.prices.length && x.prices[0],
            'Error fetching coin prices on DeFi.', 500
        );
        // tweak token
        ...
        delete x.token;
        // tweak prices
        for (let i in x.prices[0]) { x[i] = x.prices[0][i]; }
        x.price = String(x.price || '').split('');
        x.price = x.price.splice(0, x.price.length - x.accuracy).join('')
            + '.' + x.price.splice(0 - x.accuracy).join('');
        delete x.prices;
        // tweak user
        x.submitter = x.user;
        delete x.user;
        // tweak time
        x.time = new Date(parseInt(x.chain_timestamp) / 1000);
        // tweak trx_id
        x.transaction_id = x.trx_id;
        // return
        return x;
    });
    return resp;
